...
1. Прочитать разделы `docs/`, относящиеся к задаче (карта ниже). Не читать «на всякий случай» весь
   `docs/` — контекст дорог.
2. Менять только файлы, относящиеся к задаче; если нужно трогать соседние стадии — остановиться и
   спросить человека.
...
Если задача не помещается в сессию — закончить на зелёном состоянии, записать остаток в `NOTES.md`
и остановиться. Никаких «почти работает».
