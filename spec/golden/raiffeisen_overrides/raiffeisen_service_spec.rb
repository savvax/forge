...
# Generated by forge 1.0.0 for RaiffeisenService. Fixtures: fixtures.json (same directory).
require_relative 'generated_spec_helper'
require_relative 'raiffeisen_service'

RSpec.describe Provider::RaiffeisenService do
  subject(:service) { described_class.new(provider: build_record('raiffeisen'), operations: operations) }
  ...
  let(:provider_id) { dig_path(create_fixture['response_200'], ["id"]) }
  let(:create_url) { "#{described_class::BASE_URL}/payout/v1/payouts" }
  let(:overridden) { [["incomeTypeCode"]] } # source задан в overrides.yml — не сравнивается
  ...
  describe '#create_request' do
    it 'creates payout (200)' do
      ...
      stub = stub_request(:post, create_url).with(headers: auth_headers) { |req| subset_of?(without_paths(parse_body(req), overridden), expected_body) }
                                            .to_return(status: 200, body: create_fixture['response_200'].to_json,
  ...
  describe '#process_callback' do
    ...
    let(:headers) { { 'X-Api-Signature-SHA256' => sign(body, algorithm: 'sha256', encoding: 'hex') } }
    ...
    it 'approves on payout with valid signature' do
      result = service.process_callback(callback['payload'], raw_body: body, headers: headers)
      expect(result).to be_success
      expect(result.data[:status]).to eq('approved')
    end

    it 'rejects on payout' do
      failed = fixtures['callback_failed']['payload']
      failed_body = JSON.generate(failed)
      failed_headers = { 'X-Api-Signature-SHA256' => sign(failed_body, algorithm: 'sha256', encoding: 'hex') }
    ...
    it 'fails on invalid signature' do
      result = service.process_callback(callback['payload'], raw_body: body, headers: { 'X-Api-Signature-SHA256' => 'bad' })
