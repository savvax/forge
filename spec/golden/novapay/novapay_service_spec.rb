...
# Generated by forge 1.0.0 for NovapayService. Fixtures: fixtures.json (same directory).
require_relative 'generated_spec_helper'
require_relative 'novapay_service'

RSpec.describe Provider::NovapayService do
  subject(:service) { described_class.new(provider: build_record('novapay'), operations: operations) }
  ...
  let(:provider_id) { dig_path(create_fixture['response_201'], ["id"]) }
  let(:create_url) { "#{described_class::BASE_URL}/payouts" }
  let(:unmapped) { [] } # поля без источника не сравниваются
  let(:auth_headers) { { 'X-API-Key' => 'test_api_key' } }
  ...
  describe '#create_request' do
    it 'creates payout (201)' do
      expected_body = without_paths(create_fixture['request'], unmapped)
      stub = stub_request(:post, create_url).with(headers: auth_headers) { |req| without_paths(JSON.parse(req.body), unmapped) == expected_body }
  ...
  describe '#process_callback' do
    ...
    it 'approves on payout.completed with valid signature' do
      result = service.process_callback(callback['payload'], raw_body: body, headers: headers)
      expect(result).to be_success
      expect(result.data[:status]).to eq('approved')
    end

    it 'rejects on payout.failed' do
    ...
    it 'fails on invalid signature' do
      result = service.process_callback(callback['payload'], raw_body: body, headers: { 'X-NovaPay-Signature' => 'bad' })
